...
function PdfSignPage() {
  ...
  const selectedPdfUrl = useMemo(() => (selectedPdf ? `/pdfs/${selectedPdf.filename}` : null), [selectedPdf]);
  ...
  // ✅ NEW: use onLoadSuccess (page proxy) to compute viewport + unset rendering flag
  ...
  // ✅ NEW: onRenderSuccess might not provide a PDFPageProxy, so just unset the flag
